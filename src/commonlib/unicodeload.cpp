...
void parse_case(std::vector<std::string>& d,CaseMap& ca){
    if(d[12]!=""){
...
void parse_decomposition(std::string& s,Decomposition& res){
    if(s=="")return;
    auto c=split(s," ");
    size_t pos=0;
    if(c[0][0]=='<'){
...
void parse_codepoint(std::vector<std::string>& d,CodeInfo& info){
...
UnicodeData parse_unicodedata(std::vector<std::vector<std::string>>& data){
    UnicodeData ret;
    for(auto& d:data){
        CodeInfo info;
...
int main(int,char**){
    auto ld=load_unicodedata_text("D:\\MiniTools\\UnicodeDataLoader\\unicodedata.txt");
    for(auto& i:ld){
        if(i[1].size()>0xff){
            std::cout << i[1];
        }
    }
    auto data=parse_unicodedata(ld);
    std::string str;
    serialize_unicodedata(str,data);
    std::fstream("D:\\MiniTools\\UnicodeDataLoader\\unicodedata.bin",std::ios_base::binary)<<str;
    Deserializer<std::string&> de(str);
    CodeInfo info;
    deserialize_codeinfo(de,info);
    return 0;
}
