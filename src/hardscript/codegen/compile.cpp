...
bool compiler::collect_ids(std::vector<control::Control>& ctrlvec,IdTable& table){
    for(auto& it:ctrlvec){
        if(it.kind==CtrlKind::var){
            if(table.vars.count(it.name))return false;
...
#define OP_INT(op) if(ret&SIGNED)sint=leftn op rightn; if(ret&UNSIGNED)uint=leftu op rightu

#define OPERATOR_EACH(op) do{\
        OP_INT(op);\
        if(ret&FLOAT){\
            floats=leftf op rightf;\
        }\
}while(0)
...
unsigned int compiler::calcable_as_const(control::Tree* t,std::string& str,long long& sint,unsigned long long& uint,double& floats){
    ...
    SET_FLAG(ret,leftc);
    if(ret==0)return 0;
    if(t->kind==NodeKind::boolean){
        if(!calcable_bool(t,str,sint,uint,floats))return 0;
        ret|=BASE_BOOL;
    }
    else if(t->kind==NodeKind::integer){
        ret&=calcable_num(t,str,sint,uint,floats);
        ret|=BASE_INT;
    }
    else if(t->kind==NodeKind::floats){
        ret&=calcable_num(t,str,sint,uint,floats);
        ret|=BASE_FLOAT;
    }
    else if(t->kind==NodeKind::string){
        ret&=~UNSIGNED&~SIGNED&~FLOAT;
        str=t->symbol;
        ret|=BASE_STR;
    }
    else if(t->symbol=="+"){
